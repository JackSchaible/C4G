...
<asp:Content ID="Content2" ContentPlaceHolderID="Main_Content" Runat="Server">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            <%= locked ? "" : "var startDay = $(\"#StartDate select[id$='DayDDL']\").change(checkStartDate); var startMonth = $(\"#StartDate select[id$='MonthDDL']\").change(checkStartDate); var startYear = $(\"#StartDate select[id$='YearDDL']\").change(checkStartDate); var endDay = $(\"#EndDate select[id$='DayDDL']\").change(checkEndDate); var endMonth = $(\"#EndDate select[id$='MonthDDL']\").change(checkEndDate); var endYear = $(\"#EndDate select[id$='YearDDL']\").change(checkEndDate);" %>
        });
            ...
            function submitForm() {
                ...
                if (errors.length > 0) {
                    ...
                    var name = <%= locked ? "'" + di.Deal.Name + "'" : "$(\"#OfferNameTextBox\").val()" %>;
                    var description = <%= locked ? "'" + di.Deal.DealDescription + "'" : "$(\"#DescriptionTextBox\").val()" %>;
                
                    //If locked, don't output the code to grab the date
                    <%= locked ? "" : "var startDay = Number($(\"#StartDate select[id$='DayDDL'] option:selected\").text()); var startMonth = Number($(\"#StartDate select[id$='MonthDDL'] option:selected\").val()) - 1; var startYear = Number($(\"#StartDate select[id$='YearDDL'] option:selected\").text()); var endDay = Number($(\"#EndDate select[id$='DayDDL'] option:selected\").text()); var endMonth = Number($(\"#EndDate select[id$='MonthDDL'] option:selected\").val()) - 1; var endYear = Number($(\"#EndDate select[id$='YearDDL'] option:selected\").text());" %>
                    <%= locked ? "var startDate = '" + di.StartDate.ToString("dd MMM yyyy") + "';" : "var startDate = new Date(startYear, startMonth, startDay);" %>
                    <%= locked ? "var endDate = '" + di.EndDate.ToString("dd MMM yyyy") + "';" : "var endDate = new Date(endYear, endMonth, endDay);" %>
    ...
    <div class="Form">
        ...
        <%--Output the name textbox if not locked--%>
        <%= locked ? "" : "<div class=\"FormRow\"><label>Name</label><input id=\"OfferNameTextBox\" type=\"text\" maxlength=\"50\" placeholder=\"Offer Name\" value=\"" + di.Deal.Name + "\" onkeyup=\"checkName()\" onblur=\"checkName()\" oninput=\"checkName()\"/><div id=\"OfferTaken\" class=\"ErrorDiv\"></div><div id=\"OfferNameTextBoxErrors\" class=\"ErrorDiv\"></div></div>" %>
        ...
        <asp:Panel CssClass="FormRow" ID="EndDateRow" runat="server" ClientIDMode="Static">
        ...
        <div class="FormRow">
            ...
            <input type="text" ID="AbsoluteCouponLimitTextBox" maxlength="4" placeholder="25" onkeyup="checkAbsCouponLimit()"
                onblur="checkAbsCouponLimit()" oninput="checkAbsCouponLimit()" value="<%: di.Deal.AbsoluteCouponLimit %>"/>
        ...
        <div class="FormRow">
            ...
            <input type="text" ID="LimitPerCustomerTextBox" maxlength="10" placeholder="5" onkeyup="checkPCCouponLimit()"
                onblur="checkPCCouponLimit()" oninput="checkPCCouponLimit()" value="<%: di.Deal.LimitPerCustomer %>"/>
